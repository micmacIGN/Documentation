...
Suppose we have computed relative orientation for a number of subset
...
\begin{center}
\begin{tikzcd}
     W_{k^j} \arrow[rd,"p^i_{k^j}"]\arrow[r, "P_{k^j}"] & \omega   \arrow[d, "S_i" ]   \\
                                  & \omega_i
     \end
{tikzcd}
\end{center}
...
In fact, when soluton $p^i_{k^j}$  result from a local bundle adjsutment on $E^i$,
